This is a critical file. The admin.py cog differs from moderation.py in that moderation handles users and server content, while the admin cog is designed for managing the bot instance itself.
Security Warning:
By design, the commands in this file are extremely powerful. They can alter the bot's code while it's running, shut it down, or execute arbitrary Python commands.
Crucial Safeguard:
In the code below, I have included a special cog_check.
async def cog_check(self, ctx):
    # Only allow the actual application owner(s) to use commands in this cog.
    return await self.bot.is_owner(ctx.author)

This ensures that only the person who created the bot application in the Discord Developer Portal can use these commands. Even someone with the "Administrator" permission in a server cannot use them unless they are also the bot owner.
Here is the source code for internal/cogs/admin.py.
